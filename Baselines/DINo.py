...
from models.neuralop030.neuralop.layers.mlp import MLP as NeuralOpMLP
...
from Baselines.ode_model import Decoder
import numpy as np
...
class PhysicsEngine(torch.nn.Module):
    ...
    def __init__(
        ...
        latent_grid_dim = 16,
        latent_domain_lims = [[0.0, 1.0], [0.0, 1.0], [0.0, 1.0]]
    ):
        super().__init__()
        self.device = device
        ...
        self.dino = Decoder(**net_dec_params)

        self.gnot_layer = GNOT(device, trunk_size=64, branch_sizes=[3], space_dim=64, output_size=128, n_layers=1, n_head=4)
    ...
    def forward(self, data):
        ...
        latent_queries = generate_latent_queries(self.latent_grid_dim, domain_lims = [[0.0, 1.0], [0.0,1.0], [0.0, 1.0]])


        latent_queries = latent_queries.view(-1, latent_queries.shape[-1]).to(self.device)
        ...
        dino_input = latent_queries.reshape(1, 1, 16, 16, 16, 3)
        states = in_p.reshape(1, 1, 1, 4096 )
        #print(in_p.shape)
        #print(latent_queries.shape)
        model_output, _ = self.dino(dino_input, states)
        latent = model_output.reshape(4096, 128)
