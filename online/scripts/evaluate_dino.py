...
def generate_dino_video(model, x_traj, sparsity, output_name):
    frames = len(x_traj)
    print(f"Running DINo Rollout for {frames} frames...")
    
    x_ref = x_traj[0] 
    x_gt_0 = x_traj[0]
    
    indices = torch.arange(0, x_gt_0.size(0), step=int(sparsity), device=model.device)
    x_sparse_0 = x_gt_0[indices]
    
    print("Optimizing Initial Condition (z0)...")
    z_curr = model.get_initial_z(x_ref, x_sparse_0, indices)
    
    fig = plt.figure(figsize=(18, 7), facecolor='#0f0f0f')
    gs = fig.add_gridspec(2, 3, height_ratios=[6, 1], hspace=0.1)
    titles = ["Initial Condition (t=0)", "Ground Truth", "DINo Forecast (Open Loop)"]
    colors = ['#00FFFF', '#00FF00', '#FFA500']
    ...
    def update(frame_idx):
        ...
        if frame_idx > 0:
            t_span = torch.tensor([0.0, 0.01]).to(model.device)
            z_in = z_curr.squeeze(1) 
            
            with torch.no_grad():
                z_next = odeint(model.ode_func, z_in, t_span, method='rk4', options={'step_size': 0.01})
            
            z_curr = z_next[-1].unsqueeze(1) 
        
        with torch.no_grad():
            x_pred = model.decode(z_curr, x_ref)
        
        mse = F.mse_loss(x_pred, x_gt).item()
        rel_l2 = calculate_relative_l2(x_pred, x_gt).item()
        vram = get_torch_vram_usage_mb()
        
        scatters[0].set_offsets(x_sparse_0.cpu().numpy()[:, :2])
...
def main():
    ...
    default_ckpt = os.path.join(PROJECT_ROOT, "checkpoints", "dino", "dino_exact_epoch90.ckpt")
    ...
    parser.add_argument('-ckpt', type=str, default=default_ckpt, help="DINo .ckpt file")
    ...
    # HDF5 loader returns (traj_ref, traj_curr). We just need the actual sequence.
    _, traj_curr = loader.get_trajectory(sim_idx=0, max_frames=200)
    
    if not traj_curr: raise ValueError("Trajectory load failed")
    
    model = DINoEvaluator(args.enc, args.dec, args.ckpt, dev, sample_input=traj_curr[0])
    
    generate_dino_video(model, traj_curr, args.sparsity, "dino_forecast.mp4")
